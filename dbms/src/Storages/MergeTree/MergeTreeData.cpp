    ...
    log_name(log_name_), log(&Logger::get(log_name + " (Data)"))
{
    ...
    if (!attach)
    {
        format_version = MergeTreeDataFormatVersion::V1;
        WriteBufferFromFile buf(version_file_path);
        writeIntText(static_cast<int>(format_version), buf);
    }
    else if (Poco::File(version_file_path).exists())
    {
        ...
        if (version < 0 || version > 1 || !buf.eof())
            throw Exception("Bad version file: " + version_file_path, ErrorCodes::CORRUPTED_DATA);
        format_version = static_cast<MergeTreeDataFormatVersion>(version);
    }
    else
        format_version = MergeTreeDataFormatVersion::V0;
}
    ...
    MutableDataPartPtr & part, SimpleIncrement * increment, Transaction * out_transaction)
{
    ...
    DataPartsVector replaced;
    {
        ...
        String new_name;
        if (format_version == MergeTreeDataFormatVersion::V0)
